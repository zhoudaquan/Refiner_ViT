...
_logger = logging.getLogger('relabel_dataset')
...
class DatasetRelabel(data.Dataset):
...
def create_relabel_dataset(dataset_type, root, relabel_root):
    train_dir = os.path.join(root, 'train')
    if not os.path.exists(train_dir):
        _logger.error('Training folder does not exist at: {}'.format(train_dir))
        exit(1)
    return DatasetRelabel(train_dir, relabel_root)