...
def get_labelmaps(label_maps_topk, num_classes, on_value=1., off_value=0., device='cuda'):
    ...
    label_maps = torch.full([label_maps_topk.size(0), num_classes, label_maps_topk_sizes[2],
                              label_maps_topk_sizes[3]], off_value, dtype=torch.float32 ,device=device)
    for _label_map, _label_topk in zip(label_maps, label_maps_topk):
        _label_map = _label_map.scatter_(
            0,
            _label_topk[1][:, :, :].long(),
            _label_topk[0][:, :, :]*on_value
        )
        _label_map = _label_map.add_((1-torch.sum(_label_topk[0][:, :, :],dim=0,keepdim=True))/num_classes)
    return label_maps
...
def get_relabel(label_maps, batch_coords,label_size=1,device='cuda'):
        ...
        target_relabel_cls = roi_align(
            ...
            boxes=torch.cat(
                [torch.arange(num_batches).view(num_batches,
                 ...
                 batch_coords.float() * label_maps.size(3) - 0.5], 1),
            output_size=(1, 1))
        B,C,H,W = target_relabel.shape
        target_relabel = target_relabel.view(B,C,H*W)
        target_relabel = torch.cat([target_relabel_cls.view(B,C,1),target_relabel],dim=2)
...
def get_labelmaps_with_coords(label_maps_topk, num_classes, on_value=1., off_value=0.,label_size=1, device='cuda'):
    ...
    relabel = get_relabel(label_maps=label_maps,
                          ...
                          device=device)
    B,C = relabel.shape[:2]
    relabel = relabel*on_value+off_value
    if label_size==1:
        return torch.cat([ground_truth.view(B,C,1),relabel.view(B,C,1)],dim=2)
    else:
        return torch.cat([ground_truth.view(B,C,1),relabel],dim=2)
...
def mixup_target(target, num_classes, lam=1., smoothing=0.0, device='cuda', label_size=1):
    ...
    if len(target.size())>2:
        if target.size(1)==2:
            y1 = get_labelmaps(target, num_classes, on_value=on_value, off_value=0, device=device)
            y2 = get_labelmaps(target.flip(0), num_classes, on_value=on_value, off_value=0, device=device)
            ...
            y2 = y1.flip(0)
            # y2 = get_labelmaps_with_coords(target.flip(0), num_classes, on_value=on_value, off_value=off_value, device=device)
        else:
...
class FastCollateRelabelMixup(RelabelMixup):
